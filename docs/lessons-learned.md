...
Para corregir la inferencia estática de arrays filtrados, es obligatorio sustituir el filtro genérico `.filter(Boolean)` por un **Type Guard explícito** que certifique a TypeScript que el elemento ya no puede ser nulo:
  `typescript.filter((item): item is { id: string; points_earned: number } => item !== null)
...
2. **Reloj Optimista y Reactividad sin Refresh (Live Hub Fix)**:
   - Implementamos un temporizador de React (`setInterval` de 60s) en `MatchCardLive.tsx` sobre el estado `localElapsed` que avanza de a 1 minuto solo en estados activos (`1H`, `2H`, `ET`). Al recibir una actualización por realtime de Supabase, el reloj local se calibra automáticamente con el valor oficial de la BD.
   - **Desacoplamiento de Suscripciones**: Anteriormente, el realtime del Live Hub se apagaba si no había un partido en vivo cargado. Esto impedía que la UI detectara al instante cuándo un partido pasaba a estar en juego por una sincronización en caliente en segundo plano. Modificamos el diseño dividiendo las suscripciones en dos efectos independientes: un detector global de partidos en la tabla `match_results` (siempre activo, que reacciona e inserta el partido activo a la vista ante los ojos del usuario) y una suscripción dinámica secundaria de goles atada al ID del partido activo.
...
## 2026-06-14: Pérdida de Eventos en Vivo por Falta de Replicación en Supabase y Calibración Horaria Relativa con last_sync (LL-29)

### Síntoma
1. Al abrir el modal "EN VIVO" de la tarjeta del Live Hub, el reloj optimista toma el minuto inicial en que comenzó el partido en Supabase (ej. min 26). Si cerramos el modal, esperamos unos minutos y lo volvemos a abrir, el reloj retoma la cuenta desde el minuto del primer arranque (min 26), en lugar de ajustarse al minuto actual transcurrido.
2. Los goles y actualizaciones en tiempo real no llegan a las tarjetas del cliente en producción a menos que se realice un refresh completo del navegador.

### Diagnóstico
1. **Caché del Cliente REST**: Next.js o el navegador cachean las respuestas HTTP de las peticiones iniciales del cliente de Supabase JS (`/rest/v1/...`). Al cerrar y abrir el componente, se monta de nuevo y realiza la consulta REST inicial, la cual devuelve datos cacheados obsoletos, volviendo al estado inicial congelado.
2. **Reloj sin Calibración Relativa**: El reloj optimista local dependía de que Supabase reportara el `elapsed` exacto. Si el cron job de deportes se ejecuta cada 2 minutos (o tiene ligeros retrasos), el valor oficial de la base de datos se queda desfasado unos minutos respecto del tiempo real de juego del cliente.
3. **Ausencia de Replicación de Realtime**: Sintácticamente las suscripciones websocket a `match_results` y `app_settings` eran correctas, pero en la base de datos de producción de Supabase no estaba habilitada la replicación en la publicación `supabase_realtime` para esas dos tablas. Como consecuencia, el WebSocket se conectaba sin errores pero el servidor de Supabase nunca despachaba notificaciones de inserción/edición.

### Resolución (The House Way)
1. **Desactivación de Caché en Cliente Supabase**: Agregamos la cabecera `.headers({ 'Cache-Control': 'no-cache' })` a todas las consultas iniciales del cliente de Supabase (`dbMatches`, `goalData`, `goalsData`, `predData`, `resultData`) en `MatchCardLive.tsx` y `MatchPredictionCard.tsx`. Esto fuerza al navegador a ignorar la caché y traer siempre datos frescos de la BD al montar.
